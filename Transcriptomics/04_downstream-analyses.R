...
kegg.gs <- getGenesets(org = "mus", db = "kegg", cache = T, return.type = "list")
# conversion of ENSEMBL gene IDs to gene symbols
kegg.gs.list = list()
count = 0
for(pathway in kegg.gs){
  count = count + 1
  kegg.gs.list[[names(kegg.gs[count])]] <- AnnotationDbi::select(org.Mm.eg.db, keys = pathway, keytype = "ENSEMBL", columns = "SYMBOL")$SYMBOL
}
...
fgseaResTidy %>% 
  ...
  DT::datatable()
# dot plot view
ggplot(fgseaResTidy[fgseaResTidy$padj < 0.05,], aes(reorder(pathway, NES), NES)) +
